...
import { PROLOGUE_SCENARIO, STAT_LABELS } from '@leviathan/shared';
import type { ScenarioChoice, GovernmentType } from '@leviathan/shared';
...
export function PrologueScreen() {
  ...
  const handleChoice = (choice: ScenarioChoice) => {
    ...
    if (govType) {
      ...
      incrementAffinity(govType, 30);
    }

    if (choice.new_trait) addTrait(choice.new_trait);
    if (choice.new_cards) {
      for (const cardId of choice.new_cards) discoverCard(cardId);
    }

    addHistoryEntry(`[序章] 开国路径: ${choice.label}`);
    addNarrative({
      day: 0,
      ...
      <div className="relative z-10 max-w-2xl w-full px-6">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1 }}>
          <div className="text-center mb-6">
            ...
            <div className="text-dim text-xs">序章 — 旧帝国崩溃日</div>
          </div>
        </motion.div>

        <motion.div className="panel p-6 mb-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
          <div className="text-fg text-sm leading-relaxed whitespace-pre-line">
            <TypewriterText text={scenario.text} speed={30} onComplete={() => setTextDone(true)} />
          </div>
        </motion.div>

        {textDone && !chosen && (
          <motion.div className="space-y-3" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            {scenario.choices.map((choice, idx) => (
              ...
              >
                ...
                <div className="flex gap-2 mt-2 flex-wrap">
                  ...
                  {choice.new_trait && <span className="pill-tag bg-purple/15 text-purple">+特质: {choice.new_trait}</span>}
                </div>
              </motion.button>
            ))}
          </motion.div>
        ...
        {chosen && (
          <motion.div className="text-center text-gold text-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
            命运的齿轮开始转动...
          </motion.div>
        )}
      </div>
